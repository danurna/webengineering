	...
	<section role="main">
		<section id="roundinfo" aria-labelledby="roundinfoheading">
			...
			<div id="player<%= i+1 %>info">
				...
				<ul class="playerroundsummary">
					<% for (int j = 0; j < 3; j++) { %>
					<li><span class="accessibility">Frage <%= j+1 %>:
					</span><span id="player<%= i+1 %>answer<%= j+1 %>"
						class="<%=  getAnswerString(quiz.getPlayer(i).getRoundAnswer(j), "class") %>"><%=  getAnswerString(quiz.getPlayer(i).getRoundAnswer(j), "desc") %></span></li>
					<% } %>
				</ul>
			</div>
			...
			<div id="currentcategory">
				<span class="accessibility">Kategorie:</span><%= currentRound.getCategory().getName() %></div>
		</section>
		...
		<section id="question" aria-labelledby="questionheading">
			<form id="questionform" action="BigQuizServlet" method="POST">
				...
				<ul id="answers">
					...
					<li><input id="option<%= choices.indexOf(choice)+1 %>"
						type="checkbox" /><label
						for="option<%= choices.indexOf(choice)+1 %>"><%= choice.getText() %></label></li>
					<% }  %>
				</ul>
				<input type="hidden" name="action" value="nextQuestion" /> <input
					id="timeleftvalue" type="hidden" value="100" /> <input id="next"
					type="submit" value="weiter" accesskey="s" />
			</form>
		</section>
		...
		<section id="timer" aria-labelledby="timerheading">
			...
			<p>
				...
				<time id="timeleft">undefined</time>
			</p>
